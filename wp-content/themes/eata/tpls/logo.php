...
if ( $use_uploaded_logo ) {
	...
	if ( $custom_logo_image && $custom_logo_max_width ) {
		$custom_logo_max_height = ( $custom_logo_max_width / $logo_image[1] ) * $logo_image[2];
		generate_custom_style( '.logo-image', "width: {$custom_logo_max_width}px; height: {$custom_logo_max_height}px;" );
	}
	...
	if ( $custom_logo_image && $custom_logo_mobile_max_width ) {
		...
		$custom_logo_mobile_max_height = ( $custom_logo_mobile_max_width / $logo_image[1] ) * $logo_image[2];
		generate_custom_style( '.logo-image', "width: {$custom_logo_mobile_max_width}px; height: {$custom_logo_mobile_max_height}px;", "screen and (max-width: {$mobile_menu_breakpoint}px)" );
	}
}
